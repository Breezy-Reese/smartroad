...
const IncidentMap: React.FC<IncidentMapProps> = ({
  ...
  /* ------------------- Normalize center safely ------------------- */
  const safeCenter = (() => {
    try {
      return normalizeCoordinates(center);
    } catch {
    ...
    <LoadScript googleMapsApiKey={import.meta.env.VITE_GOOGLE_MAPS_API_KEY ?? ''}>
      ...
      >
        ...
        {incidents.map((incident) => {
          // Safely attempt to normalize — skip if invalid
          let position: Coordinates | null = null;
          try {
            position = normalizeCoordinates(incident.location);
          } catch {
            return null;
          }
        ...
        {selected && selected.location && (() => {
          let position: Coordinates | null = null;
          try {
            position = normalizeCoordinates(selected.location);
          } catch {
            return null;
          }
