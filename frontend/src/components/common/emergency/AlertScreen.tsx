...
const AlertScreen: React.FC = () => {
  ...
  const getSeverityMessage = (severity: string) => {
    switch (severity) {
      case 'critical':
      case 'fatal':   return 'Critical emergency — Immediate response required';
      case 'high':    return 'High severity — Urgent medical attention needed';
      case 'medium':  return 'Medium severity — Attention required';
      default:        return 'Low severity — Non life-threatening';
    }
  };
  ...
  if (!incident) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-red-600 to-red-800 flex items-center justify-center text-white text-center">
        ...
        <button onClick={() => navigate('/')} className="mt-4 bg-white text-red-600 px-6 py-2 rounded-lg">
          Go Home
        </button>
      </div>
    );
    ...
    <div className="min-h-screen bg-gradient-to-br from-red-600 to-red-800 py-8 px-4">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-4xl font-bold text-white">EMERGENCY ALERT</h1>
          <p className="text-red-200 mt-2">Incident ID: {incident._id}</p>
        </div>
        ...
        <div className="grid md:grid-cols-4 gap-4 mb-8">
          <div className="bg-white rounded-lg p-6 text-center">
            ...
            <p className="text-2xl font-bold text-red-600">{formatTime(timeElapsed)}</p>
          </div>
          ...
          <div className="bg-white rounded-lg p-6 text-center">
            ...
            <span className="px-3 py-1 rounded-full text-sm font-semibold bg-red-100 text-red-700 capitalize">
              {incident.status}
            </span>
          </div>
          ...
          <div className="bg-white rounded-lg p-6 flex flex-col items-center justify-center">
            <p className="text-sm text-gray-600 mb-3">Severity Score</p>
            <SeverityBadge
        ...
        {/* SEVERITY MESSAGE */}
        <div className="bg-white rounded-lg p-6 mb-8 text-center font-medium text-gray-800">
        ...
        <div className="bg-white rounded-lg p-6 mb-8">
          ...
          {responders.length === 0 ? (
            <p className="text-gray-500">Waiting for responders to accept...</p>
          ) : (
            <div className="space-y-3">
              {responders.map((responder) => (
                <div key={responder._id} className="border rounded-lg p-4 flex justify-between">
                  <div>
                    <p className="font-medium">{responder.name}</p>
                    <p className="text-sm text-gray-500">{responder.hospitalName}</p>
                    <p className="text-xs capitalize">{responder.responderType}</p>
                  </div>
