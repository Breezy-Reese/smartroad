...
const CHANNELS: { value: NotificationChannel; label: string; icon: React.FC<React.SVGProps<SVGSVGElement>> }[] = [
  { value: 'push',  label: 'Push',  icon: DevicePhoneMobileIcon },
...
/* ── Channel badge ── */
const ChannelBadge: React.FC<{ channel: NotificationChannel }> = ({ channel }) => {
  ...
  return (
    <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${colors[channel]}`}>
      {channel.toUpperCase()}
    </span>
  );
};
...
/* ── Add / Edit form ── */
type FormData = Omit<NextOfKin, 'id' | 'priority'>;

const EMPTY_FORM: FormData = {
  name: '', phone: '', email: '', relationship: 'Spouse', channels: ['push', 'sms'],
};
...
const ContactForm: React.FC<{
  ...
  onCancel: () => void;
  loading: boolean;
}> = ({ initial = EMPTY_FORM, onSave, onCancel, loading }) => {
...
const ContactCard: React.FC<{
  ...
  <div className="bg-white border border-gray-100 rounded-xl p-4">
    ...
    <div className="flex gap-1.5 mt-3 flex-wrap">
      {contact.channels.map((ch) => <ChannelBadge key={ch} channel={ch} />)}
    </div>
  </div>
);
    ...
    <div className="space-y-6 max-w-2xl mx-auto pb-10">
      ...
      {contacts.length > 0 && (
        <div className="bg-amber-50 border border-amber-100 rounded-xl p-4 text-sm text-amber-800">
          <strong>Notification order:</strong> Contacts are alerted by priority (#1 first). All contacts at the same escalation level are notified simultaneously.
        </div>
      )}
      ...
      <div className="space-y-3">
        {contacts.map((contact) =>
          editingId === contact.id ? (
            <ContactForm
              key={contact.id}
              initial={{ name: contact.name, phone: contact.phone, email: contact.email, relationship: contact.relationship, channels: contact.channels }}
